 ...
 if(isset($_POST['del']))
{
	...
	$rack=$_POST['rack'];
	$shelf=$_POST['shelf'];
	$delete="DELETE FROM `total_info` WHERE `field`='$field' AND `well_name`='$well' AND `doc_type`='$doc' AND `rack_num`='$rack' AND `shelf`='$shelf'";
	if($qqq=mysqli_query($con,$delete))
	{
...
if($qqq=mysqli_query($con,$select))
	...
	<th>Shelf</th>
		...
		}  echo '<th><button type="submit" name="icon" style="font-size:20px" onclick="document.getElementById("id01").style.display="block" "><i class="fa fa-edit"></i></button></th></tr>';
	}echo '<table>';
	}
}
  else
